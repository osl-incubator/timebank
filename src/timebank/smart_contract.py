...
def time_bank_contract():
    ...
    handle_creation = Seq([
        App.globalPut(time_credits, Int(0)),
        Return(Int(1))
    ])
    ...
    add_time_credits = Seq([
        App.globalPut(time_credits, App.globalGet(time_credits) + Txn.application_args[0]),
        Return(Int(1))
    ])
...
compiled_teal = compileTeal(time_bank_contract(), mode=Mode.Application, version=2)
print(compiled_teal)
